...
const ACTION_SCHEMA = {
  ...
  properties: {
    ...
    category_id: {
      type: ["string", "null"],
      description: "add_task : l'id d'une catégorie de la liste fournie, ou null si aucune ne correspond clairement. QUAND IL NOMME LA CATÉGORIE DANS SA PHRASE, TU DOIS LA RECONNAÎTRE ET LA RETIRER DU TITRE — c'est l'erreur qu'il a signalée le 7 sept. 2026, capture à l'appui : « Alexis dans la tache des leads » s'était retrouvé en TITRE, sans catégorie, et il a dû ranger la tâche à la main. Les tournures à reconnaître sont toutes celles qui désignent un rangement : « dans les leads », « dans la tâche des leads », « dans la section leads », « dans la catégorie leads », « côté administratif », « pour Melissa », « en perso ». Le nom peut être approximatif (pluriel, accent, majuscule, article) : « leads » vaut « Leads », « l'administratif » vaut « Admin ». Exemples, et le titre attendu : « ajoute Alexis dans la tâche des leads » → title « Alexis », category_id = celui de Leads ; « dans l'administratif, régler l'arnaque bancaire » → title « Régler l\'arnaque bancaire », category_id = celui d\'Admin ; « rendez-vous avec Daniel Nakache demain midi dans la section leads » → title « Rendez-vous avec Daniel Nakache », category_id = celui de Leads. Si le nom qu'il dit ne correspond à AUCUNE catégorie de la liste, laisse category_id à null et garde le titre entier plutôt que d'inventer un rangement. configure_widget : id de catégorie pour filtrer le widget, ou null pour toutes les catégories — n'inclure que si l'utilisateur a précisé une catégorie.",
    },
    max_tasks: {
      type: "number",

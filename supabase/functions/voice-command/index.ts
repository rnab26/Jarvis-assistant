...
const ACTION_SCHEMA = {
  ...
  properties: {
    ...
    contact_id: {
      type: "string",
      description: "send_message, call_contact ou transmettre_recu : id d'un contact déjà enregistré, quand la liste fournie en contient un qui correspond. Sinon utiliser contact_name.",
    },
    voice_enabled: {
      type: "boolean",
    ...
    contact_name: {
      type: "string",
      description:
        "send_message, call_contact ou transmettre_recu : le nom de la personne TEL QUE L'UTILISATEUR L'A DIT (« ma femme », « Yoni », « le plombier »), à renseigner DÈS QUE le destinataire n'est pas dans la liste de contacts fournie. Le téléphone cherchera ce nom dans le vrai répertoire, que tu ne vois pas. Ne demande donc JAMAIS un numéro avant d'avoir essayé ça.",
    },
    phone_number: {
      type: "string",
      description: "send_message, call_contact ou transmettre_recu : le numéro, uniquement si l'utilisateur l'a dicté à voix haute. Sinon utiliser contact_id.",
    },
    alarm_time: {
      type: "string",

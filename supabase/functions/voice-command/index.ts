...
const ACTION_SCHEMA = {
  ...
  properties: {
    ...
    category: {
      type: "string",
      enum: ["musique", "navigation", "messages", "ia"],
...
Pour send_message et call_contact : résous contact_id depuis la liste de contacts fournie, par nom approchant. SI TU NE TROUVES PERSONNE dans cette liste, n'abandonne pas et ne réclame pas le numéro : renvoie quand même l'action avec contact_name = le nom tel qu'il l'a dit. Le téléphone a un RÉPERTOIRE que tu ne vois pas, et c'est là que sont ses vrais contacts — la liste qu'on te donne ne contient que les fiches qu'il a dictées à Jarvis, et elle est souvent vide. Ne demande un numéro que si le téléphone t'a déjà répondu qu'il ne trouvait personne. Si le contact existe mais n'a pas de numéro (champ phone vide) et que l'utilisateur n'en a pas dicté un, utilise quand même send_message : WhatsApp demandera à qui envoyer.

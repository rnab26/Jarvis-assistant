  ...
  const boucle = async () => {
    try {
      while (!arretDemande) {
        ...
        // resterait pris, le WebSocket ouvert, et le drapeau natif jamais
        // rebaissé, donc la veille de l'AUTRE fenêtre se tairait pour toujours.
        // Même famille que le cœur resté sur « connexion », plus bas.
        if (arretDemande) {
          courante.arreter()
          return
        }
